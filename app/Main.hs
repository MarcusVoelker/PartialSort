...
handleAll :: (PrimMonad m, Ord a) => Oracle m a -> StateT (SortState m a) m ()
handleAll oracle = do
  next <- use open
  case next of
    [] -> return ()
    _ -> handleNext oracle >> handleAll oracle
...
pSort :: (PrimMonad m, Ord a) => Oracle m a -> [a] -> m (SortResult a)
pSort _ [] = return $ SortResult V.empty M.empty
pSort oracle (e0:elems) = do
  vec0 <- MV.new 1
  vec00 <- MV.new 1
  unsafeWrite vec00 0 e0
  unsafeWrite vec0 0 vec00
  let state0 = SortState (CD vec0) M.empty elems
  (SortState (CD dec) mat _) <- execStateT (handleAll oracle) state0
  decF <- deepFreeze dec
...
calcHasse :: (PrimMonad m, Ord a) => Oracle m a -> [a] -> m (Set a, Set (a,a))
calcHasse oracle elems = do
  (SortResult _ mat) <- pSort oracle elems
